...
def _format_gbp_pence(amount: object) -> str:
    try:
        pence = int(amount)
...
def cmd_report(args) -> None:
    ...
    payload = generate_nightly_report(conn, settings.timezone, notifier)
    ...
    _print_table(
        ...
        [
            ...
            ["Notification sent", "yes" if args.notify else "no"],
        ],
    )

    summary_rows = [[str(line)] for line in payload.get("summary", [])]
    if summary_rows:
        _print_table("Category Summary", ["Line"], summary_rows)
...
def cmd_pots(args) -> None:
    ...
    rows = cur.fetchall()
    table_rows = [
        [str(name or ""), str(pot_id or ""), _format_gbp_pence(balance), str(currency or "")]
